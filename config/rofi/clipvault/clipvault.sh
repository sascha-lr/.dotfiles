...
rofi_list()
{
    ...
    echo "$list" | gawk -v thumb_dir="$thumbnails_dir" '
    # Skip HTML meta tags
    /^[0-9]+\s<meta http-equiv=/ { next }
    
    # Match image binary data
    match($0, /^([0-9]+)\s(\[\[\sbinary.*(jpg|jpeg|png|bmp|webp|tif|gif).*)/, grp) {
        image = grp[1] "." grp[3]
        img_path = thumb_dir "/" image
        
        # Extract thumbnail if it does not exist
        system("[[ -f \x27" img_path "\x27 ]] || echo " grp[1] " | clipvault get > \x27" img_path "\x27")
        
        # Print Rofi format with icon
        print grp[2] "\000icon\037" img_path "\037info\037" grp[1] "\n"
        next
    }
    
    # Match text data
    match($0, /^([0-9]+)\s(.*)/, grp) {
        # Print Rofi format for text
        print grp[2] "\000info\037" grp[1] "\n"
        next
    }
    
    # Print anything else (fallback)
    1
    ' 
}
